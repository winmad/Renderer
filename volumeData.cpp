...
bool search_sss_color(std::string &name, vec3f &Sigma_a, vec3f &Sigma_s){
	/*int size = sizeof(mss) / sizeof(mss[0]);
	for(int i = 0; i < size; i++){
		if(strcmp(name.c_str(), mss[i].name) == 0){
			Spectrum s_s = Spectrum::FromRGB(mss[i].sigma_prime_s);
			Spectrum s_a = Spectrum::FromRGB(mss[i].sigma_a);
			float s[3], a[3];
			s_s.ToRGB((float*)s);
			s_a.ToRGB((float*)a);
			sigma_s = vec3f(s);
	...
	return false;*/
    for (int i = 0; i < sizeof(mss) / sizeof(mss[0]); ++i) {
        if (strcmp(name.c_str(), mss[i].name) == 0) {
            Spectrum sigma_a = Spectrum::FromRGB(mss[i].sigma_a);
            Spectrum sigma_s = Spectrum::FromRGB(mss[i].sigma_prime_s);
			std::cout << sigma_s.c[0] << ' ' << sigma_s.c[1] << ' ' << sigma_s.c[2] << std::endl;
			std::cout << sigma_a.c[0] << ' ' << sigma_a.c[1] << ' ' << sigma_a.c[2] << std::endl;
			//Sigma_s = vec3f((float*)sigma_s.c);
			//Sigma_a = vec3f((float*)sigma_a.c);
			Sigma_s[0] = sigma_s.c[0];
			Sigma_s[1] = sigma_s.c[1];
			Sigma_s[2] = sigma_s.c[2];
			Sigma_a[0] = sigma_a.c[0];
			Sigma_a[1] = sigma_a.c[1];
			Sigma_a[2] = sigma_a.c[2];
			
			std::cout << Sigma_s[0] << ' ' << Sigma_s[1] << ' ' << Sigma_s[2] << std::endl;
			std::cout << Sigma_a[0] << ' ' << Sigma_a[1] << ' ' << Sigma_a[2] << std::endl;
...
bool SpectrumSamplesSorted( float *lambda,  float *vals, int n) {
    for (int i = 0; i < n-1; ++i)
...
float AverageSpectrumSamples( float *lambda,  float *vals,
...
RGBSpectrum SampledSpectrum::ToRGBSpectrum()  {
    float rgb[3];
    ToRGB(rgb);
...
SampledSpectrum SampledSpectrum::FromRGB( float rgb[3],
                                         SpectrumType type) {
    SampledSpectrum r;
	...
	SampledSpectrum color = r.Clamp();
	float m_rgb[3];
	color.ToRGB(m_rgb);
	std::cout << m_rgb[0] << ' ' << m_rgb[1] << ' ' << m_rgb[2] << std::endl;
    return r.Clamp();
}


SampledSpectrum::SampledSpectrum( RGBSpectrum &r, SpectrumType t) {
    float rgb[3];
    r.ToRGB(rgb);
...
void Blackbody(const float *wl, int n, float temp, float *vals) {
    ...
    float norm = powf(555.0, 5.0) * (exp(C2/(555.0*temp)) - 1.);
    for (int i = 0; i < n; ++i)
        vals[i] = float(norm/(powf(float(wl[i]), 5.0)*(exp(C2/(wl[i]*temp))-1.)));
}
